...
In this section, the algorithm is presented for Problem~\ref{problem:workspace_decomposition}. The main algorithm is described first followed by the description of smaller sub-procedures.

The main procedure is shown in Algorithm~\ref{alg:optimization_procedure}. The optimization process is an interative pair-wise re-optimization procedure. The process terminates when the maximum cost in the partition is minimized. The pair-wise optimality is ensured via a linear search over a sampled search space. The whole process is started off with an initial solution.

As an input, a polygon representing $\mathbb{C}$ is provided . A set of $n$ initial positions of $n$ robots is also provided as one of the inputs in the form of $(x,y)$ coordinates. As an initialization step, the polygon $\mathbb{C}$ is partitioned into $n$ subregions of equal area by the anchored area partitioning algorithm~\cite{hert1998polygon}. This step is designed to provide the algorithm  with a starting point to be used in the iterative re-optimization procedure.

An adjacency graph $\mathcal{G}$ is computed on the resultant partition. For every cell in the partition, an approximation of the coverage cost is computed using metric $\chi$. For every cell with the highest cost, the following re-optimization steps are performed. First, an adjacent cell with the lowest cost is picked. These two adjacent cells are modified according to Algorithm~\ref{alg:equitable_cut}. The result of Algorithm~\ref{alg:equitable_cut} are two cells where the maximum cost is minimized. The cost and the partition are modified accordingly. The loop is repeated for the next cell with the highest cost in the decomposition. The termination condition for this loops is when the maximum cost over all cells in the decomposition stops decreasing. Algorithm~\ref{alg:optimization_procedure} demonstrates the described procedure.
...
Algorithm~\ref{alg:equitable_cut} desribes a procedure for recomputing a cut that would minimize the maximum cost. In this procedure, a search for a cut is performed over a cut space. The cut space is samples with $K$ points. The point that minimizes the maximum cost is selected and a cut is performed.
\begin{algorithm}
	\caption{$\text{equitable\_cut}(w_a, w_b)$}
	\label{alg:equitable_cut}
	\begin{algorithmic}[1]
		\REQUIRE Polygon $w_a=\{Z_0,Z_1,\ldots\}$, Polygon $w_b=\{Z_0,Z_1,\ldots\}$
			\STATE $c\gets$ shared edge between $w_a$ and $w_b$.
			\STATE $v_s,v_e\gets$ endpoints of $c$.
			\STATE $w_{\text{temp}}\gets w_a\cup w_b$.
			\STATE $S\gets$ cut space from $v_s$.
			\IF{$\chi(w_a)>\chi(w_b)$}
				\STATE dir$\gets$ CCW
			\ELSE
				\STATE dir$\gets$ CW
			\ENDIF			
			\FOR{each $v_i\in S$ in direction dir}
				\STATE $w^{'}_a,w^{'}_b\gets$ cut ($v_s,v_i$)
				\IF{dir=CW and $\chi(w^{'}_a)>\chi(w^{'}_b)$}
