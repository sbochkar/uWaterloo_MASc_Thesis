...
We shall now derive an interesting result regarding the total length of the straight line segments, which will assist us with the formulation of the problem. In much of literature, one of the most common types of paths used for coverage path planning are sweeping paths consisting of a series of parallel straight lines. The benefits of such arrangement is that there is no overlap between lines leading to efficient paths in terms of length per area covered. Suppose a system when a robot can teleport between straight line segments in effect ignoring the transition segments. One would like to find the total length of such a path. As it turns out, the length of such a path as directly related to the area of the workspace. We shall establish this by recalling the concepts of a Riemann sum.
...
Suppose the coverage path consists of a series of parallel line segments spaces distance $\delta x$ apart. This shares some similarities with a concept of integrals. As such, the area is equal to 
\begin{equation}
	A\approx \sum \ell\delta x=\delta x\sum\ell
\end{equation}

In other words, the total length of the path is approximated by the area. That is,
\begin{equation}
	\ell_t=\sum\ell_i\approx\frac{A}{\delta x}.
\end{equation}

The important side effect of this is that there is no relation to the orientation of these parallel lines. That is these lines could be reoriented. The number of lines could go up or down, but the total length is going to remain approximately constant. The error can be studied by analyzing the error attributed to a Riemann sum, which is a well studies subject. 
The error is \ldots

Finally, let us take a look at the cost again in equation~\ref{eq:segmented_cost}. This tells us that the first term is going to be approximately constant given that the area is covered in the most efficient manner without any overlap. Hence, th 
