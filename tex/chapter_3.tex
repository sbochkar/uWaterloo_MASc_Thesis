...
In this section, the problem statement is developed. We start with the statement of the general problem. We then make a couple of assumptions and justify these assumptions. The final problem statement is then stated.
...
\end{equation}
whereas the cost of a transition segment has both linear and angular component, i.e,
\begin{equation}
...
The problem statement is modified accordingly.
...
Recall the structure of the cost function of a segmented path from equation(~\ref{eq:segmented_cost}) and the property of Boustrophedon paths where transition segments have an roughly fixed cost. The problem becomes one of minimizing the number of straight line segments $n$ to achieve complete coverage. For convex polygons, this problem is solved by Huang~\cite{Huang2001optimal}.
...
Moreover, this metric also gives the angular component of the whole path. Specifically, 
\begin{equation}
...
For each $P_i$, a number of parallel lines is generated in an orientation that minimizes the altitude. The result is a set of lines, $\Gamma$. Now the problem becomes that of choosing an order and direction to traverse each line with, i.e., a tour of $\Gamma$. Observe that each line can be traversed with two directions. Hence, the second subproblem can be defined as follows:
